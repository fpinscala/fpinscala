...
trait These[+A, +B] {
  ...
  def mapL[A2, B2 >: B](f: A => A2): These[A2, B2] = bimap(f, identity)

  def mapR[A2 >: A, B2](f: B => B2): These[A2, B2] = bimap(identity, f)
...
object These {
  ...
  /* 
   * Zips together the two `Seq`s, returning the remaining elements
