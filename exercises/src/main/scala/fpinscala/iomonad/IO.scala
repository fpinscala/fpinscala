...
object IO3 {
  ...
  */
  ...
  def translate[F[_], G[_], A](f: Free[F, A])(fg: F ~> G): Free[G, A] = {
    ...
    val t = new (F ~> ({type f[a] = Free[G, a]})#f) {
      override def apply[A](f: F[A]): Free[G, A] = Suspend(fg(f))
    }

    runFree[F, ({type f[a] = Free[G, a]})#f, A](f)(t)(freeMonad[G])
  }
  ...
  def read(file: AsynchronousFileChannel,
    ...
    (cb: Either[Throwable, Array[Byte]] => Unit) => {
      ...
      file.read(buffer, fromPosition, (), new CompletionHandler[Integer, Unit] {
        override def completed(read: Integer, ignore: Unit): Unit =
          cb(Right(buffer.slice(0, read).array()))

        override def failed(exc: Throwable, ignore: Unit): Unit = cb(Left(exc))
      })
    }
  }
...
object test extends App {
  ...
  val app = for {
    _ <- IO3.Console.printLn("yo, type something ...")
    str <- IO3.Console.readLn
    _ <- str match {
      case Some(value) => IO3.Console.printLn(s"echo: $value")
      case None => IO3.Console.printLn("oops ...")
    }
  } yield ()
